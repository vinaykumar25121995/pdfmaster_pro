...
exports.extractEntity = exports.summarizeDocument = exports.chatWithDocument = void 0;
...
if (geminiApiKey) {
    try {
        aiClient = new genai_1.GoogleGenAI({ apiKey: geminiApiKey });
...
const runOfflineExtractor = (text, task, query) => {
...
const extractEntity = async (req, res) => {
    ...
    if (!aiClient) {
        ...
        return res.json({ result: fallback[type], source: 'offline-heuristics' });
    }
    try {
        let prompt = '';
        if (type === 'contacts') {
        ...
        const fallback = runOfflineExtractor(documentText, type);
        return res.json({ result: fallback[type], source: 'offline-fallback' });
    }
};
